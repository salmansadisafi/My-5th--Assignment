// ১. সব এলিমেন্ট সিলেক্ট করা (আধুনিক const দিয়ে)
...
// এই অ্যারের মান পরে পরিবর্তন হতে পারে, তাই এখানে let ব্যবহার করা হয়েছে
let allIssues = [];

// ২. API থেকে ডাটা লোড করার ফাংশন
function loadIssues() {
...
function openModal(issue) {
    ...
    modalBody.innerHTML = "<div class='mb-6'>" +
        "<h2 class='text-2xl font-bold text-slate-800 mb-4'>" + (issue.title || 'No Title Provided') + "</h2>" +
        "<div class='flex flex-wrap items-center gap-3 text-xs font-medium text-slate-500'>" +
            "<span class='px-3 py-1 rounded-full " + statusBg + "'>" + statusText + "</span>" +
            "<span>• Opened by <span class='font-bold text-slate-700'>" + (issue.author || 'Anonymous') + "</span></span>" +
    ...
    "<div class='bg-slate-50 p-5 rounded-xl border border-slate-100 mb-6'>" +
        "<p class='text-sm text-slate-600 leading-relaxed'>" + (issue.description || 'No description available for this issue.') + "</p>" +
    "</div>" +
    "<div class='grid grid-cols-2 gap-8 border-t border-slate-100 pt-6'>" +
        "<div>" +
            "<p class='text-[10px] text-slate-400 mb-1 uppercase font-bold tracking-wider'>Label:</p>" +
            "<p class='text-sm font-bold text-slate-800'>" + (issue.label || 'BUG').toUpperCase() + "</p>" +
        "</div>" +
        "<div>" +
            "<p class='text-[10px] text-slate-400 mb-1 uppercase font-bold tracking-wider'>Priority:</p>" +
            "<p class='text-sm font-bold text-slate-800'>" + (issue.priority || 'LOW').toUpperCase() + "</p>" +
        "</div>" +
    "</div>";
...
// মডাল ক্লোজ করার বাটন ইভেন্ট
closeModalBtn.addEventListener("click", function () {
    issueModal.classList.add("hidden");
    issueModal.classList.remove("flex");
});

// ৫. ট্যাব ফিল্টারিং ফাংশন
